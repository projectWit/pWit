	...
	var _currentMenuNum = 0;
		var gnb11stEvent = {
				...
				Event : function(slider, settings){
					...
					$('.btn_totalcate', slider).on("click",function(){
						if(!$("#gnbCateAllLayerWrap", slider).attr("mode")){
							$('.btn_totalcate', slider).css({'background-position': "-" + (slider.width()+1) + "px 0px"});
							
							$("#gnbCateAllLayerWrap", slider).attr("mode", "open");
							$(".allcategory", slider).show();
						}else{
							$('.btn_totalcate', slider).css({'background-position': "0px 0px"});
							
							$("#gnbCateAllLayerWrap", slider).attr("mode", "");
							$(".allcategory", slider).hide();
						}
					});
