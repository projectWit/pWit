...
<form action="">
		<div id="wrap">
			...
			<%
				...
				System.out.println("계정이름출력 : "+arr.get(i).getaName());} */
				%>
			...
			<table class="p_table" summary="">
				...
				<tbody>
					<%
						/* String[] str1 = { "매입", "매출", "외상매입", "외상매출", "미수금", "자산", "부채",
								"자본", "미지급금", "비용" };
						String[] str2 = { "매입계정", "매출계정", "외상매입계정", "외상매출계정", "미수금계정",
								"유동, 비유동자산이 있으며 부채과 자본을 합친 것이 자산계정이다.",
								"자산에서 갖고 있는 부채를 나타낸다.",
								"자산에서 부채를 뺀 실제 자산을 나타내며 운영자금으로 주식이 대표적이다.", "미지급금계정",
								"비용계정" }; */
								for (int i = 0; i < arr.size(); i++) {
					%>
					<tr>
						<td class="p_td"><%=arr.get(i).getaCode()%></td>
