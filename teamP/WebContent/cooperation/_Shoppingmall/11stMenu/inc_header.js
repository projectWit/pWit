...
HeaderGnb.getChannel = function() {
	...
	if (url.indexOf("www.11st.co.kr/mart11st/") != -1 || url.indexOf("www.11st.co.kr/html/martMain.html") != -1){
		return 'MART';
		...
		return 'SOHO';
	} else if (url.indexOf("tour.11st.co.kr/html/vertical/tourMain.html") != -1 || typeof(TOUR_HEADER) != 'undefined' ){
	 	return 'TOUR';
...
HeaderGnb.makeGnb = {
	area : '',
	isLogin : funcCheckIsLogin() || funcCheckIsSSOLogin(),
	...
	makeMenuLUtilElement : function() {
		...
		var setChannel = function(){
			if ( _this.channel == 'MART' ){
				jQuery("#lUtilMart").html('<strong>마트</strong>');
			} else if ( _this.channel == 'TOWN' ){
				jQuery("#lUtilTown").html('<strong>타운</strong>');
			} else if ( _this.channel == 'TOUR'){
				jQuery("#lUtilTour").html('<strong>여행</strong>');
			} else if ( _this.channel == 'DEAL' ){
			 	jQuery("#lUtilShockingDeal").html('<strong>쇼킹딜</strong>');
		...
		var logoUtilChannel = ['SOHO', 'TOWN', 'DEPARTHD', 'DEPARTFS', 'SHOP', 'DEAL'];
		var logoClassName = 'navi_11st';
		if( jQuery.inArray(_this.channel, logoUtilChannel) >= 0 ) {
			logoClassName = 'navi_11st_ico';
		}
		...
		element.push('		<li><a id="lUtilShockingDeal" href="http://deal.11st.co.kr/html/nc/deal/main.html" onclick="goStatUrl(this.href, \'' + _this.areaCodePre + 'CT0102\');return false;" class="navi_shockdeal">쇼킹딜</a></li>');
		element.push('		<li><a id="lUtilBook" href="http://book.11st.co.kr/index.html" onclick="goStatUrl(this.href, \'' + _this.areaCodePre + 'CT0103\');return false;" class="navi_book">도서</a></li>');
		...
		element.push('		<li><a id="lUtilMart" href="http://www.11st.co.kr/html/martMain.html" onclick="goStatUrl(this.href, \'' + _this.areaCodePre + 'CT0106\');return false;" class="navi_mart">마트</a></li>');
		element.push('	</ul>');
		...
		jQuery(function($){
			...
			jQuery('#go11st_wrap > button').bind('click', function(){
				...
				if( _this.channel != 'TOUR' ) {
					...
					element.push('				<li><a href="#" onclick="ShortcutGnb(\'' + _this.areaCodePre + 'UT0103\');return false">바로가기<br>설치하기</a></li>');
					...
					element.push('				<a href="http://www.11st.co.kr/browsing/CouponPlace.tmall?method=getDirectBenefit&addCtgrNo=939791&naviTab=2" onclick="doCommonStat(\''+_this.areaCodePre+'UT0107\')" class="defbtn_med mdtype4"><span>혜택 상세보기</span></a>');
					element.push('			</div>');
				} else {
	...
	makeMenuRUtilElement : function() {
		...
		if(this.isLogin) {
			...
			element.push('		<li><a href="'+_GNB_CONTEXT_PATH_+'/register/memberSubscribeForm.page" onclick="join(\''+_this.areaCodePre+'UT0301\');return false;">회원가입</a></li>');
		}
		...
		if(this.channel == 'TOUR') {
		...
		} else if(this.channel == 'TOWN') {
			element.push('		<li class="ut_my11st" id="utMy11st"><a href="'+ _TOWN11ST_URL_+'/myTown/main.tmall?method=getMain" onclick="doCommonStat(\'TMA0306\');">나의11번가</a>');
			element.push('		<div>');
			element.push('			<ul id="mYInFo_11st">');
			element.push('				<li><a href="'+_ORDER_HIS_URL_ +'" onclick="doCommonStat(\'TMA0307\');">주문배송조회</a></li>');
			element.push('				<li><a href="'+_TOWN11ST_URL_ +'/myTown/myShopList.tmall?method=getMyShopListMain" onclick="doCommonStat(\'TMA0309\');">즐겨찾기상점</a></li>');
			element.push('				<li><a href="'+_TOWN11ST_URL_ +'/myTown/myCouponList.tmall?method=getMyCouponListMain" onclick="doCommonStat(\'TMA0310\');">즐겨찾기쿠폰</a></li>');
			element.push('			</ul>');
			element.push('		</div>');
			element.push('		</li>');
			element.push('		<li><a href="'+ _SHOPPINGCART_URL_ +'" onclick="doCommonStat(\'TMA0305\');">장바구니</a></li>');
			element.push('		<li><a href="http://help.11st.co.kr" onclick="doCommonStat(\'TMA0311\');">고객센터</a></li>');
		} else {
			...
			element.push('				<li><a href="'+_GNB_CONTEXT_PATH_ +'/register/getGradeInfo.tmall?method=getGrade" onclick="doCommonStat(\''+_this.areaCodePre+'UT0404\');">구매등급혜택</a></li>');
			element.push('				<li><a href="'+_GNB_CONTEXT_PATH_ +'/browsing/AuthChipExchPlace.tmall?method=getCustBenefitPage&subTabIdx=3" onclick="doCommonStat(\''+_this.areaCodePre+'UT0405\');">클럽혜택</a></li>');
		...
		if(this.channel == 'TOWN') {
			element.push('		<li><a href="http://town.soffice.11st.co.kr" onclick="doCommonStat(\'TMA0312\');">타운셀러오피스</a>');
		} else {
			...
			element.push('				<li><a href="http://town.soffice.11st.co.kr" onclick="goStatUrl(this.href, \''+_this.areaCodePre+'UT0703\');return false;">타운셀러오피스</a></li>');
	...
	makeCornerNaviElement : function() {
		...
		element.push('	<ul class="best_li">');
		element.push('		<li class="bst_li1"><a href="#" onclick="point(\''+_this.areaCodePre+'GN0201\');return false"><span>이벤트/쿠폰/혜택</span></a></li>');
		element.push('		<li class="bst_li2"><a href="#" onclick="best(\''+_this.areaCodePre+'GN0202\');return false"><span>11번가랭킹</span></a></li>');
		element.push('		<li class="bst_li3"><a href="http://www.11st.co.kr/browsing/AuthChipExchPlace.tmall?method=getCustBenefitPage&subTabIdx=3" onclick="doCommonStat(\''+_this.areaCodePre+'GN0203\');"><span>11번가클럽</span></a></li>');
		element.push('		<li class="bst_li4"><a href="#" onclick="shopping(\''+_this.areaCodePre+'GN0204\');return false"><span>열린쇼핑정보</span></a></li>');
		element.push('	</ul>');
	...
	makeSearchElement : function() {
		...
		element.push('		<form name="GNBSearchForm" id="AKCFrm" autocomplete="off" action="http://search.11st.co.kr/SearchPrdAction.tmall" method="get" onSubmit="goSearch(); return false;" target="_top">');
	...
	if (this.area == "search") {
		...
		element.push('							</ul>');
		element.push('							<li><a href="#" onclick="setSearchTarget(\'SEMANTIC\');return false"><em>시맨틱검색</em></a></li>');
		element.push('							<li><a href="#" onclick="setSearchTarget(\'CONTENTS\');return false"><em>쇼핑리뷰</em></a></li>');
		element.push('							<li><a href="#" onclick="setSearchTarget(\'MODEL\');return false"><em>가격비교</em></a></li>');
	...
	if (this.area=="search") {
		...
		element.push('					<input type="text" class="text_search adver" name="kwd" value="" id="AKCKwd" onFocus="clearAdUrl();" onKeyPress="if ( event.keyCode == 13) goSearch(); " style="ime-mode:active;">');
	}else {
		element.push('					<input type="text" class="text_search adver" name="kwd" value="" id="AKCKwd" onfocus="clearAdUrl();searchManager.init(true);">'); //tInSearch AKCKwd
	}
	...
	if (this.area != "extra") {
		...
		element.push('			</div>');
		element.push('		</form>');
		element.push('		<div id="gnbTxtAd_divId" title="MA01I"><button type="button" class="btn_search" id="gnbTxtAd" onclick="goSearch(); return false;"><span>검색</span></button></div>');
		element.push('</fieldset>');

		if (this.area == 'search') {
			if (noSemanticResult || noModelResult || noContentsResult) {
				var pageText = '시맨틱';
				if(noModelResult) pageText = '모델';
	...
	makeHtml : function() {
		...
		headerElement.push('		<div id="defaultHeader2">');
...
HeaderGnb.categoryNavi = {
	...
	makeNaviElement : function(area) {
		...
		element.push('		<div class="navigate">');
		...
		element.push('				</div>');
		element.push('			</div>');
		element.push('			<div class="gnb_menu3" id="gnbMenu3">');
		...
		element.push('					</li>');
		element.push('					<li class="gnbm8"><a href="#gnb_cate_layer8" onClick="return false;"><span><span>도서/취미</span></span></a>');
		...
		element.push('					</li>');
		element.push('					<li class="gnbm10"><a href="#gnb_cate_layer10" onClick="return false;"><span><span>현대백화점</span></span></a>');
	...
	if(_this.area == 'main') {
		...
		element.push('	</div>');
	    element.push('	<script type="text/javascript">');
		element.push('	HeaderComm.SimpleFIOUtil.setSelector("#gnbMenu3 li[class^=gnbm]");');
		element.push('	</script>');
	...
	makeCategoryAllLayer : function() {
		var element = [];
		var _this = this;
		var $ctgrWrap = jQuery('<div class="category2">');
		for(var grpIdx=1 ; grpIdx <= 8 ; grpIdx ++) {
			var grpData = eval("FooterData.metaCtgrGrp" + grpIdx);
			if(grpData != undefined) {
				var $wrap = jQuery('<div class="viewcate'+grpIdx+'">');
				var $ctgrList = jQuery('<ul/>');
				//var $promoCtgrList = jQuery('<span/>');

				for(var idx = 0 ; idx < grpData.length ; idx++) {
					var data = grpData[idx];
					if(data.CtgrLv == 1) {
						$wrap.append('<h3><span>'+data.CtgrNm+'</span></h3>');
					} else if(data.CtgrLv == 3) {
						if(data.CtgrKindCd == '03') { //Promotion Category
							//$promoCtgrList.append(this.setCtgrNmStyle(data));
						} else {
							$ctgrList.append(this.setCtgrNmStyle(data));
						}
					}
				}
				//var promotionCtgrCnt = jQuery('li', $promoCtgrList).size();
				var ctgrCnt = jQuery('li', $ctgrList).size();

				//$ctgrList.append(jQuery('li', $promoCtgrList));
				$wrap.append($ctgrList);
				$ctgrWrap.append($wrap);
				jQuery('li:gt(9)', $wrap).remove(); // limit 10 Category
		...
		//인기 전문관
		if(FooterData.popularMallList != undefined) {
			var $wrap = jQuery('<div class="popular_special" />');
		...
		//인기코너
		if(FooterData.popularCornerList != undefined) {
			var $wrap = jQuery('<div class="popular_corner2" />');
	...
	makeGrpCtgrLayer : function(grpIdx) {
		...
		if(grpData != undefined) {
			for(var idx = 0 ; idx < grpData.length ; idx++) {
				var data = grpData[idx];
				if(data.CtgrLv == 2) {
					$wrap.append('<h3>' + data.CtgrNm + '</h3>').append('<ul/>');
				} else if(data.CtgrLv == 3) {
					jQuery('ul:last-child', $wrap).append(this.setCtgrNmStyle(data));
				}
			}
			//통계코드
/*			jQuery('ul > li', $wrap).bind('click', function() {
		...
		if(jQuery('.banner', $layerWrap).size() > 0) {
			...
			$closeBtn.bind('click', function() {
				jQuery('#gnbMenu3 > ul > li').mouseleave();
			});
		}
	...
	setCtgrNmStyle : function(ctgrInfo) {
		...
		if(ctgrInfo.CtgrKindCd != '03') { //전시카테고리
			ctgrLink = $URL.ctgr(1, ctgrInfo.RefCtgrNo, ctgrInfo.RefCtgrCd);
		}
	...
	initEvent : function() {
		...
		//카테고리 전체보기 버튼
		jQuery('button.btn_totalcate', _this.$allLayerWrap).bind('click', function(e) {
		...
		$menuList.each(function(idx){
			...
			$this.bind({
				mouseenter : function(e){
					_currentMenuNum = idx;
					var isEmpty = jQuery('.gnb_inner', '#gnb_cate_layer' + (idx+1)).is(':empty');
					if(isEmpty) {
						_this.makeGrpCtgrLayer(idx);
						//통계코드
						var codeIdx = (idx+2) >= 10 ?(idx+2) : '0' + (idx+2);
						doCommonStat(_this.areaCodePre + 'CG' + codeIdx+ '01');
					}

					$this.addClass('selected');
					$sub.show();
					if(_this.area == 'main') {
						$gnbArrow.css('display','block');
		...
		$gnbArrow.bind({
			mouseover : function(e){
				$menuList.eq(_currentMenuNum).mouseover();
			}
		});
	},
	init : function(area){
