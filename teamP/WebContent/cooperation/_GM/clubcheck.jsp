...
function SendClubNo(cId,cName) {
	window.opener.frmInput.cId.value=cId;
	window.opener.frmInput.cName.value=cName;
	window.close();
}
</script>
<body>
<%String id=request.getParameter("mId");
  ...
  System.out.println(result.get(0).getcId());
  %>
...
<table class="notice_board" cellspacing="0" cellpadding="0"	valign="middle" style='border:1px solid #bbc5ce; font-family:맑은 고딕;'>
	...
	<tbody>
		...
		<tr onclick="SendClubNo('<%=result.get(i).getcId()%>','<%=result.get(i).getcName() %>')" style="cursor:pointer;"><td><%=result.get(i).getcName() %></td><td><%=result.get(i).getsName() %></td></tr>
		<%} 
		}%>
	</tbody>
</table>
