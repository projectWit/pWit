...
function detailLecture(pageUrl, lecId) {
	location.href="/teamP/detail.gm?id="+pageUrl+"&lecId="+lecId;
}
</script>
<body>
				...
				<!-- INLINE NOTICE --> 
				 ...
				 String pageUrl="";
				  if(member.getmId().equals("admin_gm") || member.getmId().equals("admin")) {
					  pageUrl="WIT_GM_worker";
				  } else {
					  pageUrl="Member_GM_Lecture";
				  }
				 %>
				<!-- LIST REPEAT --> 
				<c:forEach var="item" items="${list }"  varStatus="status">
				<tr class="bl_oddline" onclick="detailLecture('<%=pageUrl %>','${item.lecId }')" style="cursor:pointer;">
					<td class='bl_list bl_no'>${item.lecId}</td>
