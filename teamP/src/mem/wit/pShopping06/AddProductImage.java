 ...
 */
@WebServlet("/teamP/Shopping6/AddProductImage.do") 
public class AddProductImage extends HttpServlet {
	...
	protected void doPost(
		...
		System.out.println("서블릿 진입 addProductImage");
		//응답콘텐츠타입을 JSON형태로 지정 
		...
		try {
			...
			//임시 저장폴더
			diskFactory.setRepository(new File(contextRootPath + "/Shopping6/upload/temp"));
			...
			System.out.println("상품 시퀀스 코드 : "+prd_code);
			String img1 = photoList.get(0).getUploadedFileName();
			String img2 = photoList.get(1).getUploadedFileName();
			
			AService tst = new AService();
			int su = tst.addProductImage(prd_code, img1, img2);
...
/*			Gson gson = new Gson();
	...
	private void processUploadFile(
		...
		//저장할 절대경로로 파일 객체 생성
		File uploadedFile = new File(contextRootPath + "/Shopping6/upload/" + uploadedFileName); 
	...
	private void processFormField(RequestModel model, FileItem item) throws Exception{
		...
		} else if("multi_sel_id".equals(name)) {
			sellerID = value;
		} else if("multi_prd_code".equals(name)) {
			prd_code = Integer.parseInt(value);
		}
	}
