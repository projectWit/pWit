...
public class GMController {
	...
	public ModelAndView clubInsert(HttpServletRequest request, HttpSession session) {
	...
	public ModelAndView leclist(HttpServletRequest request) {
		...
		String query = "select l.lecId, l.lecName, e.eKName, ty.tName, p.place, sp.sName, s.sSDay, s.sEDay, l.lecPay, e.eTel "
				+ "from GM_Lecture l, GM_Sche s, GM_Teacher t, Employee e, C_PTime pt, C_Place p, C_Sport sp, C_Type ty "
				+ "where l.lecId=s.lecId and p.pId=l.pId and e.eId=t.eNum and t.tId=s.tId and s.pId=pt.pId and sp.sId=l.sId and ty.tId=l.tId";
	...
	public ModelAndView qSetInsert(HttpSession session) {
		...
		List<QBoardDTO> list = (List<QBoardDTO>) m
				.select("searchName",	"select mName from WitMember where mId='"+member.getmId()+"'");
		mv.addObject("list", list);
		return mv;
	}
